...
def add_volume_joint(parent):
    ...
    vj_name = get_new_name(parent, "VJ_bind")
    ...
    jnt.setAttr("translateX", 0.002)
    set_override_color(jnt, .0001)
...
def add_partial_joint(sibling):
    ...
    jnt_name = get_new_name(sibling, "partial_bind")
    ...
    for ctrl in ctrl_hierarchy:
        ...
        if not pm.objExists(name):
            ...
            if not name.endswith(("ref", "armBase_Rt_loc", "armBase_Lt_loc")):
                zero_rotation(node)
            if circle_dict is not None and name.endswith("_anim") and name in circle_dict:
