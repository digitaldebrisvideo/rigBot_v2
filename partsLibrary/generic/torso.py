...
class Torso(standardPart.StandardPart):
    """Biped or quad FK / IK torso. Uses cmSplineChain plugin node.
    ...
    def __init__(self):
        ...
        self.add_option('numberJoints',
                        ...
                        default = 6,
                        tool_tip='Numder of torso joints.',
                        min=0)
    ...
    def build_guide(self, **kwargs):
        ...
        #
        # for p in param_nodes:        spline.create_stretch_crv (spline_crv)
        ...
        # Create cog
        cog_ct_zero, cog_ctrl = self.guide_ctrl('cog', shape='cog', color='yellow', scale=[3,3,3], axis=X)
        ...
        fk_name = 'chest_FK'
        fk_zero, fk_ctrl = self.guide_ctrl(fk_name, driver=chest_name, allow_offset_ctrls=0, create_pivot=0, shape='pin_circle', color='light_blue', scale=[2]*3)
        mc.xform(fk_ctrl+'.cv[*]', r=1, ro=[-90,0,0])
        fk_zeros.append(fk_zero)
        fk_ctrls.append(fk_ctrl)
    ...
    def build_rig(self):
        ...
        surf = spline.create_surface_mtx(spline_driver_nodes, name=prefix+'_torso', parent=noxform_grp, width=width, stretch=inverse_plugin)
            ...
            if use_plugin:
                ...
                for p in params:
                    ...
                    for a in ['stretch','reverseRoot']:
                        mc.connectAttr(chest_ctrl+'.'+a, p+'.'+a)
        ...
        spline.ramp_scale([hip_ctrl]+mid_ctrls+[chest_ctrl], [hip_jnt]+jnts, interpolation=4, driver_attr='torsoScale')
        ...
        # tag spaces
        spaces.tag(chest_ctrl, add_parent_space=False, arg="fk:"+mid_ctrls[-1]+" ik:"+cog_ctrl)
        ...
        for ctrl in mid_ctrls:
            ...
            if i+1 < len(mid_ctrls):
                ...
                print mid_ctrls[i],mid_ctrls[i+1]
            else:
