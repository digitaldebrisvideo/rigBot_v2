...
class BipedLeg(standardPart.StandardPart):
    ...
    def build_guide(self, **kwargs):
        ...
        # ik fk switch ctrl
        switch_zero, switch_ctrl = self.guide_ctrl(name='leg_IK_switch', shape='pin_gear', color='lavendar', driver=leg_end_jnt)
        ...
        if make_bendy:
            baCtrl = self.guide_ctrl(name='bendyLeg_A', shape='star', color='pink', axis='X')
            bbCtrl = self.guide_ctrl(name='bendyLeg_B', shape='star', color='pink', axis='X')
            bcCtrl = self.guide_ctrl(name='bendyLeg_C', shape='star', color='pink', axis='X')
    ...
    def build_rig(self):
        ...
        if up_leg_twist_jnts:
            ...
            ikChain.upper_twist(utils.get_parent(up_leg_ik_jnt), up_leg_ik_jnt, lo_leg_ik_jnt, up_leg_jnt, lo_leg_jnt, up_leg_twist_jnts)
        ...
        if make_bendy:
            ...
            mc.pointConstraint(up_leg_ik_jnt, lo_leg_ik_jnt, bendyLegACtrl[1]+'_CONST', mo=1, n=bendyLegACtrl[1]+'_CONST_pc')

            mc.parentConstraint(lo_leg_ik_jnt, bendyLegCCtrl[0], mo=1, n=bendyLegCCtrl[0]+'_prc')
            mc.pointConstraint(lo_leg_ik_jnt, leg_end_ik_jnt, bendyLegCCtrl[1]+'_CONST', mo=1, n=bendyLegCCtrl[1]+'_CONST_pc')

            mc.parentConstraint(lo_leg_ik_jnt, bendyLegBCtrl[0], mo=1, n=bendyLegBCtrl[0]+'_prc')
            oc = mc.orientConstraint(up_leg_ik_jnt, lo_leg_ik_jnt, bendyLegBCtrl[1]+'_CONST', mo=1, n=bendyLegBCtrl[1]+'_CONST_pc')[0]
            mc.setAttr(oc+'.interpType', 2)
            ...
            ikChain.biped_bendy(bendy_joints_drivers, bendy_ctrls, False, noxform_grp, 'smoothKnee', ctrl_driver=bendyLegBCtrl[1])
        ...
        jnts = [up_leg_jnt]+up_leg_twist_jnts+[lo_leg_jnt]+lo_leg_twist_jnts+[leg_end_jnt]
