...
class EncoreHand(standardPart.StandardPart):
    ...
    def __build_finger_guide(self, number_joints, name, color, length=1.0):
        ...
        for i, jnt in enumerate(jnts[:-1]):
            ...
            zero, ctrl = self.guide_ctrl(name=name + '_' + letter, shape='cube', scale=[0.3] * 3,
    ...
    def build_rig(self):
        ...
        for attr in attrs:
            for jnt in bind_jnts:
                ...
                for a in ['x', 'y', 'z']:
                    mc.setDrivenKeyframe(ctrl_grp + '.r' + a, cd=md + '.output' + a.upper(), itt='spline', ott='spline',
                                         dv=0, v=0)
                    mc.setDrivenKeyframe(ctrl_grp + '.r' + a, cd=md + '.output' + a.upper(), itt='spline', ott='spline',
                                         dv=10, v=1)
