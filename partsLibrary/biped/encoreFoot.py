...
class EncoreFoot(standardPart.StandardPart):
    ...
    def build_guide(self, **kwargs):
        ...
        # Crete IK toe ctrls
        ball_zero, ball_ctrl = self.guide_ctrl(name='toe_IK', shape='circle', color=color, driver=jnts[1], axis='X',
                                               create_pivot=False)
        mc.xform(ball_ctrl + '.cv[*]', r=1, t=[mirror_value * 0.25, 0, 0])

        # Create toe ctrl
        ball_fk_zero, ball_fk_ctrl = self.guide_ctrl(name='toe_FK', shape='circle', allow_offset_ctrls=0,
                                                     color='light_blue', driver=jnts[1], axis='X')
    ...
    def build_rig(self):
        ...
        ankle_offset_zero, ankle_offset_ctrl, ankle_offset_offsets, ankle_offset_last_node = self.anim_ctrl(name,
                                                                                                            match_position=ankle_jnt)
        ...
        orient_loc = mc.createNode('transform', n='{0}_orient_loc'.format(prefix), p=reverse_ball_ctrl)
