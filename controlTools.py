...
import re
import json
from rigBot import common
from rigBot import utils.set_attrs (
...
def reload_control_data(verbose=False):
    """Reload control and shape data from env"""
    ...
    # system_base_path = os.path.dirname(utils.__file__)
    # system_base_path = os.path.dirname ('L:\\3D\\scripts\\rigging_dept\\exchange\\rigBot\\config')
    system_base_path = os.path.dirname ('L:\\3D\\scripts\\rigging_dept\\rigArtists\\bhamilton\\rigBot\\rigBot\\config')
...
def makeControl(name='', shape='cube', color='yellow', jointCtrl=False, axis='Y',
           size=[1,1,1], snapTo=None, unlock_attrs='', constraints=0):
    """Create control hierarchy

        KWARGS:
            :name: ctrl name (NOTE: suffix will automatically be appended based on setting, if snapTo node has default suffix
                                it will be replaced with grpnames and ctrl suffix)

            :shape: shape of ctrls  (see config file for list of all shapes 
            NOTE:  shape='circle_ext' will create circle with history, instead of shape library

            :color: color of ctrl   (see config file for list of all colors)

            :axis: direction of ctrl  (x, y, or z) default is y

            :jointCtrl: {int} is this a joint control or not, default to False

            :snapTo: node to snap and rotoate the ctrlGrp to

            :unlock_attrs: IF set initial lock an hide will be set on all other attrs

            :constraints {int}  -- [type of constraint, if any, to constrain the snapTo node to anim ctrl
                                    options are : 0=None , 1=parent, 2=point 3.orient 4.point/orient] (default: {0})"""
    ...
    if color is not None:
        if type(color) is list:
            data=color
            color=None
        elif color is not None and color not in colors:
    ...
    if not name:
        if snapTo:
            name = snapTo
            if str(name.rfind('_bind')) not in '-1':
                    name = name.replace('_bind', '')
            if str(name.rfind('_jnt')) not in '-1':
                name = name.replace('_jnt', '')
            if str(name.rfind('_loc')) not in '-1':
                name= name.replace('_loc', '')
            if str(name.rfind('_fol')) not in '-1':
                name= name.replace('_fol', '')  
            if str(name.rfind('_ctrl')) not in '-1':
                name= name.replace('_ctrl', '')    
            if str(name.rfind('_anim')) not in '-1':
                name= name.replace('_anim', '_ctrl_anim')                      
            if str(name.rfind('_bind')) in '-1' and str(name.rfind('_jnt')) in '-1' and str(name.rfind('_loc')) in '-1' and str(name.rfind('_fol')) in '-1'and str(name.rfind('_ctrl')) in '-1':
                name = name 
            # print name
    ...
    if mc.objExists (name+'_anim'):
        alls=mc.ls ('*'+name+'*_anim')
        if alls:
            n=len(alls)+1
            ctrlpre=name+'_'+str(n)+'_'
            ctrlname=ctrlpre+'anim'
            name=ctrlname
            # print (name+'_anim exists finding unique name')
    else :
        ctrlpre=name+'_'
        ctrlname=name+'_anim'
        name=ctrlname
        # print ('creating '+ctrlname)
    ...
    if jointCtrl:
        ctrl = mc.createNode('joint', name=ctrlname)
        mc.setAttr (ctrl+'.radius', 0)
        mc.setAttr (ctrl+'.drawStyle', 2)

    if history:
        ctrl=mc.circle(ch=1, o=1, nr = [x, y, z], n = ctrlname)[0]

    else:
        ctrl = mc.createNode('transform', name=ctrlname)
    ...
    # create att and par
    ctrls = []
    grpNames = ['grpOffset','ctrlGrp']
    # if makePivot:
    #     grpNames = ['grpOffset','grp','par', 'pivot']

    for i in range(len(grpNames)):
        grp = mc.createNode('transform', n=ctrlpre+grpNames[i])
        if i > 0:
            mc.parent(grp, ctrls[-1])
        ctrls.append(grp)

    if not snapTo:
        mc.setAttr (ctrlpre+'grpOffset.rx', 0)
        mc.select (ctrlpre+'grpOffset')


    ctrls.append(ctrl)
    mc.parent(ctrls[-1], ctrls[-2])



    if not history:
        shps=create_shape(shape, ctrl, axis, scale=size)
    if not data:
        set_color(color, ctrl)
    else:
        set_color(ctrl, data=data)


    # position to reference
    if snapTo:
        mc.xform(ctrls[0], ws=1,t=mc.xform(snapTo, q=1,ws=1,t=1))
        mc.xform(ctrls[0], ws=1,ro=mc.xform(snapTo, q=1,ws=1,ro=1))

        if jointCtrl:
            mc.parent(ctrls[-1], w=1)
            mc.xform(ctrls[0], ws=1, ro=[0,0,0])
            mc.parent(ctrls[-1], ctrls[-2])

    if constraints == 1:
        utils.set_attrs (snapTo, 't r s ud', l=0, k=1)
        mc.parentConstraint (ctrls[-1], snapTo, mo=0)
        # print ('parentConstraint '+snapTo+' to '+ctrls[-1])


    if constraints == 2:
        utils.set_attrs (snapTo, 't r s ud', l=0, k=1)
        mc.pointConstraint (ctrls[-1], snapTo, mo=0)
        # print ('pointConstraint '+snapTo+' to '+ctrls[-1])

    if constraints == 3:
        utils.set_attrs (snapTo, 't r s ud', l=0, k=1)
        mc.orientConstraint (ctrls[-1], snapTo, mo=0)
        # print ('or '+snapTo+' to '+ctrls[-1])

    if constraints == 3:
        utils.set_attrs (snapTo, 't r s ud', l=0, k=1)
        mc.pointConstraint  (ctrls[-1], snapTo, mo=0)
        mc.orientConstraint  (ctrls[-1], snapTo, mo=0)
        # print ('point and orient constraint '+snapTo+' to '+ctrls[-1])
...
def create_shape(shape='square', ctrls=[], axis='x', add=False, translate=[0,0,0], rotate=[0,0,0], scale=[1,1,1], data=[]):
    ...
    for ctrl in ctrls:
        ...
        axis = axis.upper()
        if axis == '-Y':
            mc.xform(cvs,  ro=[0,0,180])
        elif axis == 'X' :
            mc.xform(cvs,  ro=[0,0,-90])
        elif axis == '-X'  :
            mc.xform(cvs,  ro=[0,0,90])
        elif axis == 'Z'  :
            mc.xform(cvs,  ro=[90,0,0])
        elif axis == '-Z'  :
            mc.xform(cvs,  ro=[-90,0,0])

        mc.xform(cvs, r=1, t=translate)
        # mc.xform(cvs, r=1, ro=ro)
...
def create_offset_ctrls(ctrl=None, num_offsets=1, vis_attr=None, match_shape=[]):
    ...
    offset_suffix = '_grpOffset'
    ctrl_suffix = '_ctrlGrp'
    ...
    for i in range(num_offsets):
        ...
        off_ctrl_name = utils.join_strings([base_name, offset_suffix])
...
def create_movable_pivot(ctrl=None, shape='diamond', ctrl_type='transform'):
    ...
    # different legacy method which works for joint ctrls wit hjoint orient values
        ...
        zero =  utils.get_parent(ctrl+'_ctrlOff')
        ...
        mc.parent(ctrl+'_ctrlGrp', par_grp)
...
def get_unique_name(name):
    ...
    i = 0
    new_name = utils.clean_name(name.replace('#',''), keep_underscores=True)
...
reload_control_data(verbose=True)
...
#         ctrl = get_unique_name(name.replace('_anim','')+'_#_anim')

#     else:
#         if mc.objExists(name+'_anim'):
#             mc.warning('Control already exists! Using a unique name.')

#         ctrl = get_unique_name(name+'_#_anim')

#     prectrl=name
#     if ctrl.endswith ('_anim'):
#         prectrl=ctrl.replace ('_anim', '')

#     zero = prectrl+'_ctrlGrp'
#     off = prectrl+'_ctrlOff'

#     # Create nodes
#     zero = mc.createNode('transform', n=zero)
#     # con = mc.createNode('transform', n=con, p=zero)
#     # mocap = mc.createNode('transform', n=mocap, p=con)
#     off = mc.createNode('transform', n=off, p=zero)
